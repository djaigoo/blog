...
以下实现**在原数组上排序**，仅使用一个与原数组等长的临时数组做合并，结果写回 `arr`，不返回新切片。
...
// MergeSort 归并排序（原数组上排序，结果写回 arr）
...
// mergeSort 对 arr[l:r) 进行归并排序，tmp 为与 arr 等长的临时数组
func mergeSort(arr, tmp []int, l, r int) {
    ...
    mergeSort(arr, tmp, mid, r)
    mergeToArr(arr, tmp, l, mid, r)
}

// mergeToArr 将 arr[l:mid) 与 arr[mid:r) 两段有序区间合并回 arr[l:r)，用 tmp 做中转
func mergeToArr(arr, tmp []int, l, mid, r int) {
    copy(tmp[l:r], arr[l:r])
    i, j, k := l, mid, l
    for i < mid && j < r {
        if tmp[i] <= tmp[j] {
            arr[k] = tmp[i]
